...
Status (25 September 2026): the read-only relay is deployed at `https://biomaster-catalogue.polihronovnikola.workers.dev/catalogue` on the confirmed Workers Free ($0) plan. Its source is `scripts/cloud/catalogue-worker.mjs`. The Cloudflare cloud test fetched all 105 products with 105 unique IDs. GitHub's `CATALOGUE_RELAY_URL` variable points to that permanent relay; production access and the first complete Actions/Pages run are being verified. The Mac updater remains enabled in parallel as an independent fallback, as requested.
