...
Status (25 September 2026): the snapshot service is deployed at `https://biomaster-catalogue.polihronovnikola.workers.dev/catalogue` on the confirmed Workers Free ($0) plan. Its scheduled handler fetched and stored all 105 products in the free `biomaster-catalogue-snapshots` KV namespace. The complete [Actions/Pages run](https://github.com/Polihronos/dr-biomaster-shop-calculator/actions/runs/36114426107) passed (54 focused tests, zero price/catalogue mismatches, three public-offer review flags) and deployed. The first timer-triggered refresh is still being verified; the initial snapshot was created using the dashboard scheduled-handler test. GitHub's `CATALOGUE_RELAY_URL` variable points to this service. The Mac updater remains enabled in parallel as an independent fallback, as requested.
