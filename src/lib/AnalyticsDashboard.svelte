<script lang="ts">
	...
	import type { DirectoryHandleLike } from '$lib/daily-sales';
	...
	let directory: DirectoryHandleLike | null = $state(null);
	...
	async function initialize() {
		loading = true;
		try {
			directory = await restoreAnalyticsRoot();
			if (!directory) return;
			if ((await analyticsPermission(directory, false)) !== 'granted') {
	...
	async function connect() {
		...
		error = '';
		try {
			if (directory && needsPermission) {
				const permission = await analyticsPermission(directory, true);
				if (permission !== 'granted') throw new Error('Достъпът до Documents не беше разрешен.');
			} else {
				directory = await chooseAnalyticsRoot();
			}
	...
	async function refresh() {
		...
		error = '';
		try {
			result = await loadAnalytics(directory, products);
			if (!result.filesRead) error = 'В избраната папка няма TXT файлове в daily orders или daily sales.';
		} catch (caught) {
			if (/разрешение/iu.test(messageFor(caught))) needsPermission = true;
			error = messageFor(caught);
		} finally {
			loading = false;
		}
	...
	<section class="analytics-modal" aria-label="Анализи на продажби и поръчки" transition:fly={{ y: 18, duration: 180 }}>
		...
		{#if !directory || needsPermission}
			<div class="connect-state">
				...
				<div>
					<strong>{needsPermission ? 'Разреши отново достъп до Documents' : 'Свържи папка Documents'}</strong>
					<p>Ще прочета всички TXT файлове от <code>daily orders</code> и <code>daily sales</code>. Данните не се изпращат никъде.</p>
				</div>
				<button class="primary" onclick={connect} disabled={loading}>{loading ? 'Чета…' : needsPermission ? 'Разреши достъп' : 'Избери Documents'}</button>
			</div>
		{:else}
